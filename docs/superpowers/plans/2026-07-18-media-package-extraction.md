...
- **Renderer examples vs the media entry side-effect (import ordering) — RESOLVED DURING IMPLEMENTATION:** the original assumption ("setup runs in the module body after imports, so it always wins") was **wrong** for the actual examples: they use `import "@smoove/renderer/register"` — a side-effect *import* that runs `setupServerRendering()` at import time, **before** the later `import { Audio } from "@smoove/media"` side-effect, which then clobbered the Node factories with browser sources (symptom: mediabunny `UrlSource` fed a filesystem path → `ERR_INVALID_URL` → silent hang). **Fix (in the package, not the examples):** `@smoove/media`'s entry gates its registration on `!detectEnvironment().isRendering`. `register`/`setupServerRendering()` sets the `__KONVA_MOTION_RENDERING__` flag before media's side-effect evaluates, so media skips; in the reverse order (media first) the renderer's setup simply overwrites. Either import order lands on the Node factories.
