...
packages/effects       @smoove/effects: hardware-accelerated shader effects. An `effects: [...]` prop on any node/Sequence (9 filters: blur, colorKey, glow, water, ...), 20 generative ShaderSource nodes (MeshGradient, Metaballs, Waves, GodRays, Voronoi, ...), and 3 ImageShaderSource logo animations (Heatmap, LiquidMetal, GemSmoke — CPU-preprocessed edge/blur fields), ported from paper-design/shaders (Apache-2.0, see NOTICE). One shared WebGL2 context in the browser; headless-gl via @smoove/renderer/gl on the server. konva and core are peerDeps.
...
- **`core` extends Konva classes.** `Composition extends Konva.Stage`,
  ...
  not a regular dep, so consumers pin the version. The demo pins a real `konva`
...
- **`@smoove/studio` is the reusable UI layer**; the `demo` app is just a
  registry and routing shell around it. Don't put studio-shaped UI logic
