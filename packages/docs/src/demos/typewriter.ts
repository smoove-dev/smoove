...
import type { HighlightConfig } from "@konva-motion/core";
...
const typeThenHl = new Text({
  ...
  width: colW,
  text: PHRASE,
  typewriter: { mode: "letter", durationInFrames: 140 },
  highlights: [hl],
  fontSize: 20,
  lineHeight: 1.5,
  fontFamily: FONT,
...
const makeColumn = (x: number, reserve: boolean) => {
  ...
  col.add(
    new Text({
      width: "100%",
      text: PARA,
      typewriter: { mode: "letter", durationInFrames: 220, reserveHeight: reserve },
      fontSize: 14,
      lineHeight: 1.4,
      fontFamily: FONT,
...
main.register((local) => {
  // Sweep the panel-2 highlight in once typing (140 frames) has finished.
  hl.progress = interpolate(local, [156, 216], [0, 1], {
