...
`@smoove/core` ships a synchronous flexbox engine and a flex-aware
...
from `@smoove/core`. smoove is the primary API surface.
</Callout>
...
```ts
import { Flex, Block, Rect, Circle, Text } from "@smoove/core";
