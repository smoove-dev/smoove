...
description: The smoove documentation template — every content component, themeable in light and dark.
---

This page is the living style reference for smoove docs. Every content element below is a building block — write it in plain MDX and it inherits this rhythm, spacing, and color automatically, in both light and dark themes.
...
import { Timeline, easeOut } from "smoove";
...
Pages can embed a running example with the `<smoove-player>` web component from
`@smoove/player`. The element is registered in the browser, so it hydrates after
load and plays a composition like an HTML5 `<video>`:

```html
<smoove-player src="/demos/pulse.js" controls autoplay loop></smoove-player>
```
