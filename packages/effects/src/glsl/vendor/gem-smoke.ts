// Derived from paper-design/shaders packages/shaders/src/shaders/gem-smoke.ts (Apache-2.0). See NOTICE.
...
vec2 gaussBlur9x9RG(sampler2D tex, vec2 uv, vec2 dudx, vec2 dudy, float radius) {
  ...
  float k[9];
  k[0] = 1.0; k[1] = 8.0; k[2] = 28.0; k[3] = 56.0; k[4] = 70.0;
  k[5] = 56.0; k[6] = 28.0; k[7] = 8.0; k[8] = 1.0;
  vec2 sum = vec2(0.0);
...
void main() {
  ...
  if (u_isImage == true) {
    ...
    vec2 blurred = gaussBlur9x9RG(u_texture, imageUV, dudx, dudy, 10.);
    roundness = 1. - blurred.x;
    vec2 texelA = 1.0 / u_resolution;
    float k3[3];
    k3[0] = 1.0; k3[1] = 2.0; k3[2] = 1.0;
    for (int j = -1; j <= 1; ++j) {
      for (int i = -1; i <= 1; ++i) {
        imgAlpha += k3[i + 1] * k3[j + 1] * texture(u_texture, imageUV + vec2(float(i) * texelA.x, float(j) * texelA.y)).g;
      }
    }
    imgAlpha /= 16.0;
  } else {
