// Derived from paper-design/shaders packages/shaders/src/shaders/perlin-noise.ts (Apache-2.0). See NOTICE.
...
vec3 gradientPredefined(float hash) {
  int idx = int(mod(floor(hash * 12.0), 12.0));
...
float p_noise(vec3 position, int octaveCount, float persistence, float lacunarity) {
  ...
  float maxValue = 0.0;
  octaveCount = int(clamp(float(octaveCount), 1.0, 8.0));
