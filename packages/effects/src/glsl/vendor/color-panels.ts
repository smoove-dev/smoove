// Derived from paper-design/shaders packages/shaders/src/shaders/color-panels.ts (Apache-2.0). See NOTICE.
...
void main() {
  ...
  for (int set = 0; set < 2; set++) {
    ...
    for (int i = 0; i <= 20; i++) {
      ...
      int colorIdx = int(mod(float(idx), float(colorsCount)));
      int nextColorIdx = int(mod(float(idx + 1), float(colorsCount)));
    ...
    for (int i = 0; i <= 20; i++) {
      ...
      int colorIdx = int(mod(float(colorsCount) - mod(float(idx), float(colorsCount)), float(colorsCount)));
      if (colorIdx < 0) colorIdx += colorsCount;
      int nextColorIdx = int(mod(float(colorIdx + 1), float(colorsCount)));
