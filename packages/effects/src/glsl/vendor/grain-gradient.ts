// Derived from paper-design/shaders packages/shaders/src/shaders/grain-gradient.ts (Apache-2.0). See NOTICE.
...
void main() {
  ...
  if (u_shape > 3.5) {
    ...
    grain_uv = grain_uv * graphicRotation;
    grain_uv *= u_scale;
    grain_uv -= graphicOffset;
    grain_uv *= u_resolution;
    grain_uv *= .7;
  } else {
    shape_uv = .5 * ((v_uv - .5) * u_resolution / 100.);
    grain_uv = 100. * ((v_uv - .5) * u_resolution / 100.);
    ...
    if (u_fit > 0.) {
      ...
      grain_uv /= (patternBoxNoFitBoxWidth / u_resolution.x);
    }
    vec2 patternBoxScale = vec2(1.);
