...
// Mip-free stand-in: our runtime never builds mipmaps, so the gradient
// arguments of upstream's textureGrad are irrelevant.
vec4 textureGrad2(sampler2D t, vec2 uv, vec2 dx, vec2 dy) {
  return texture(t, uv);
}

uniform sampler2D u_texture;
uniform float u_time;
uniform mediump float u_textureAspectRatio;
...
float blurEdge3x3(sampler2D tex, vec2 uv, vec2 dudx, vec2 dudy, float radius, float centerSample) {
  ...
  sum += w2 * textureGrad2(tex, uv + vec2(0.0, -r.y), dudx, dudy).g;
  sum += w2 * textureGrad2(tex, uv + vec2(0.0, r.y), dudx, dudy).g;
  sum += w2 * textureGrad2(tex, uv + vec2(-r.x, 0.0), dudx, dudy).g;
  sum += w2 * textureGrad2(tex, uv + vec2(r.x, 0.0), dudx, dudy).g;

  sum += w1 * textureGrad2(tex, uv + vec2(-r.x, -r.y), dudx, dudy).g;
  sum += w1 * textureGrad2(tex, uv + vec2(r.x, -r.y), dudx, dudy).g;
  sum += w1 * textureGrad2(tex, uv + vec2(-r.x, r.y), dudx, dudy).g;
  sum += w1 * textureGrad2(tex, uv + vec2(r.x, r.y), dudx, dudy).g;
...
void main() {
  ...
  vec4 img = texture(u_texture, imgUV);
  ...
  img[1] = blurEdge3x3(u_texture, imgUV, dudx, dudy, 8., img[1]);
