...
// procedural hash noise instead of the upstream noise texture, highlights
// gated by content coverage (node-scoped captures, not a full-plane surface).
...
void main() {
  ...
  // smoove adaptation: gate highlights by content coverage, so a node-scoped
  // capture doesn't paint caustics over the empty padding around the node.
  float surface = clamp(opacity, 0., 1.);

  causticNoise = max(-.2, causticNoise);

  float hightlight = .025 * u_highlights * causticNoise * surface;
  hightlight *= u_colorHighlight.a;
  color = mix(color, u_colorHighlight.rgb, .05 * u_highlights * causticNoise * surface);
  opacity += hightlight;
