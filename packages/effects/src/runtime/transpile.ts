 ...
 * the form every effect fragment is authored in — to GLSL ES 1.00, so it
 * can run on a WebGL 1 context (e.g. `headless-gl` during server rendering,
 * which has no WebGL2). The effect fragments follow a single, regular pattern:
 *
 ...
 *   prepended (the runtime enables the extension on WebGL1 contexts)
 *
 * Paired with {@link VERTEX_SHADER_100}. Kept deliberately string-based (no real
 * GLSL parse) — it only needs to cover the shapes effect fragments are written in.
 */
